...
def create_samples_list(params):
    ...
    if scenario == 'co_event_coh_only':
        ...
        for idx, row in train_val_df.iterrows():
            ...
            train_val_samples.append((row['s1'], row['pre_event_grd'], row['pre_event_coh'], row['co_event_coh'], row['s2_lbl']))
...
def create_samples_list_hand_lbl(params):
    ...
    if scenario == 'co_event_coh_only':
        ...
        for idx, row in test_df.iterrows():
            try:
                test_samples.append((row['s1'], row['pre_event_grd'], row['pre_event_coh'], row['co_event_coh'], row['hand_lbl']))
            except:
                test_samples.append(
